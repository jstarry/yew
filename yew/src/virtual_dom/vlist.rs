...
impl VDiff for VList {
    ...
    fn expand(
        ...
        let mut right_children = match ancestor {
            ...
            Some(VNode::VList(vlist)) => vlist.children.iter_mut().collect(),
        ...
        if lefts_keyed && rights_keyed {
            ...
            right_children = right_children
                .into_iter()
                .filter(|right| matched_keys.contains(right.key().as_ref().unwrap()))
                .collect();
        ...
        self
            .children
            .iter_mut()
            .for_each(|left| left.expand(parent_scope, rights.next()));
    }
    ...
    fn apply(
        ...
        let mut nodes: Vec<NodeRef> = self
            ...
            .map(|left| {
                ...
                let node = left.apply(parent, new_next_sibling.clone(), ancestor);
