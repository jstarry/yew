...
use crate::html::{Component, ComponentUpdate, MountOptions, NodeCell, Scope};
use std::any::TypeId;
...
type Generator<COMP> = dyn FnOnce(GeneratorType, Scope<COMP>, bool) -> Mounted;
...
impl<COMP: Component> VComp<COMP> {
    ...
    {
        let generator =
            move |generator_type: GeneratorType, parent: Scope<COMP>, sync: bool| -> Mounted {
                ...
                match generator_type {
                    GeneratorType::Mount(element, ancestor) => {
                        ...
                        let mut scope = scope.mount_in_place(
                            ...
                            props,
                            MountOptions {
                                ancestor: Some(VNode::VRef(ancestor)),
                                occupied: Some(occupied.clone()),
                                sync,
                            },
                        );
                    ...
                    GeneratorType::Overwrite(type_id, scope, occupied) => {
                        ...
                        scope.update(ComponentUpdate::Properties(props), sync);
...
impl<COMP: Component> Unmounted<COMP> {
    ...
    fn mount<T: INode>(
        ...
        (self.generator)(GeneratorType::Mount(element, ancestor), env, sync)
    }

    /// Overwrite an existing virtual component with a generator.
    fn replace(self, type_id: TypeId, old: Mounted, env: Scope<COMP>, sync: bool) -> Mounted {
        (self.generator)(
            ...
            MountState::Unmounted(this) => {
                ...
                let mounted = match reform {
                    Reform::Keep(type_id, mounted) => {
                        // Replace the currently rendered component by updating the properties.
                        // Always update synchronously since we may already be updating the parent
                        // asynchronously.
                        this.replace(type_id, mounted, env.clone(), true)
                    }
                    Reform::Before(before) => {
                        ...
                        // Always mount synchronously since we may already be updating the parent
                        // asynchronously.
                        this.mount(parent, node, env.clone(), true)
                    }
                };
